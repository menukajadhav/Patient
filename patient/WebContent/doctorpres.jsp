...
String strDateNew1 = sdf1.format(now);
	...
	String e = request.getParameter("pstatus");
	String f = request.getParameter("prescription");
	...
	try {
		...
		ps = con.prepareStatement("INSERT INTO prescription(pid,name,des,reason,pstatus,prescription,dstatus,date)VALUES(?,?,?,?,?,?,?,'"+strDateNew1+"')");
...
<%
    ...
    String serverstatus ="Visit";
		
										
				try{
				Connection	conn=databasecon.getconnection();
				PreparedStatement ps1=conn.prepareStatement("Update public set dstatus=? where pid='"+fid1+"'");
				ps1.setString(1,serverstatus);
				int x=ps1.executeUpdate();
				
				response.sendRedirect("doctorpage.jsp?message=success");
				}
				catch (Exception ex) 
				{
